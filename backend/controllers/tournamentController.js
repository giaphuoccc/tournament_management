...
export const createTournament = async (req, res) => {
    ...
    try {
        ...
        if (isNaN(parsedTimeStarted.getTime()) || isNaN(parsedTimeEnded.getTime())) {
            return res.status(400).json({ message: 'Invalid date format for timeStarted or timeEnded' });
        }
...
export const getTournamentByGameId = async (req, res) => {
    try {
        ...
        if (!tournaments.length) {
            return res.status(404).json({ message: "Không có giải đấu cho game này!"});
        }
